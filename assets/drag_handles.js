...
(function() {
  ...
  function clampDraggedNm(entry, bound, candidateNm) {
    ...
    if (bound === 'lower') {
      return Math.min(candidateNm, upper - MIN_GAP_NM);
    }
    if (bound === 'upper') {
      return Math.max(candidateNm, lower + MIN_GAP_NM);
    }
    return null;
  }
  ...
  function getPlotRectPixels(bounds) {
    ...
    const left = plot.margin.l;
    const top = plot.margin.t;
    const width = plot.width - plot.margin.l - plot.margin.r;
    const height = plot.height - plot.margin.t - plot.margin.b;
  ...
  function findNearestEdgeFromEvent(evt) {
    if (!llEntries.length) return null;
  ...
  function hideDragPreview() {
    if (dragPreview.layer) {
      dragPreview.layer.style.display = 'none';
    }
  }
  ...
  function updateDragPreview(regionIdx, bound, newNm) {
    ...
    const edgeX = (bound === 'lower') ? plotRect.left + loPxRel : plotRect.left + hiPxRel;
  ...
  function onDocumentDragUp(evt) {
    ...
    if (Number.isFinite(newNm) && window.dash_clientside && window.dash_clientside.set_props) {
  ...
  function onSvgMouseUp(evt) {
    if (!drawState.active || drawState.startNm === null || drawState.endNm === null) {
      return;
    }
  ...
  function setupPopoverButtons() {
    ...
    if (acceptBtn) {
      acceptBtn.addEventListener('click', () => {
        ...
        if (Number.isFinite(lo) && Number.isFinite(hi) && window.dash_clientside && window.dash_clientside.set_props) {
  ...
  function onPlotlyAfterPlot() {
    // No explicit SVG handles to render. Keep hook to preserve timing/debug behavior.
  ...
  function attachPlotlyListeners(attempt) {
    ...
    if (!graph || typeof graph.on !== 'function') {
      if (attempt < 60) {
        setTimeout(() => attachPlotlyListeners(attempt + 1), 200);
      }
      return;
    }

    if (plotlyListenersBound) {
      return;
    }
  ...
  function init() {
    ...
    // Capture phase is required because Plotly drag layers may stop propagation.
